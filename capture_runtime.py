...
def normalize_capture(raw, role, board, profile, plan):
    ...
    meta={'source':'HARDWARE','role':role,'plan':copy.deepcopy(plan),'profile':copy.deepcopy(profile),
        ...
        'acquisition':{'source':'ODRIVE_ONBOARD','requested_hz':(plan or {}).get('capture_rate_hz') or rate,'native_control_loop_hz':rate,
            'rate_note':'odrive high_rate_capture records at the native control-loop rate; no decimation is applied. requested_hz is the plan request (native when blank).',
            'actual_hz':(len(rows)-1)/(rows[-1]['time_s']-rows[0]['time_s']) if rows[-1]['time_s']>rows[0]['time_s'] else None,
            'dropped_samples':max(0,cap['samples']-len(rows)),'boards_share_clock':False,
            'timing_uncertainty_s':{'within_board':'one control cycle (1/native rate) quantisation','between_boards':None,
                'note':'Each board triggers on its own clock; inter-board alignment is unknown.'},
